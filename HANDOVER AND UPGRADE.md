...
**תאריך:** אוגוסט 2026 (עדכון אחרון: 05/08/2026 - מימוש פיצ'ר חדש "המלצת פיזור מזומן חדש" (סעיף 12) + הוספת ROADMAP אופציונלי לשיפורים עתידיים (סעיף 13))
**שני תת-פרויקטים קיימים במקביל** (ראו סעיף 1 להבחנה ביניהם).

**עדכון אחרון בקצרה (למי שקורא במהירות):** נוסף פיצ'ר עצמאי חדש - "המלצת פיזור מזומן חדש": מנוע AI חדש (`engine/cash_allocation.py`), endpoint חדש (`POST /recommendations/cash-allocation`) עם cooldown, מסך Frontend עצמאי (`CashRecommendation.jsx`) וניווט (Navbar) בין המסכים. פרטים מלאים בסעיף 12. (לפני כן, בוצע תיקון באג קריטי בהזרקת פרופיל המשקיע + עיצוב מחדש של שדות הבחירה - ראו סעיף 11.) **בנוסף, נוסף סעיף 13 - ROADMAP מרוכז של שיפורים אפשריים לעתיד (אופציונלי בלבד, ללא החלטה או עדיפות מחייבת).**
...
|---|---|---|
| 1 | **Deployment בפועל** (Render + Vercel) | כרגע הכל רץ רק מקומית (`localhost`) - זו התלות המרכזית שחוסמת גם משתמשים אמיתיים וגם ארוז ל-PWA/store (ראו סעיף 5.3, סעיף 7 סוף). ללא זה, כל שאר הפריטים ברשימה הזו תיאורטיים. |
| 2 | **CI/CD בסיסי** (GitHub Actions) | הרצת syntax check / `pytest` אוטומטית בכל push, כדי לתפוס שגיאות לפני שהן מגיעות ל-production - במיוחד רלוונטי לאור המשמעת הידנית הקיימת של "test before send" (סעיף 6, פריט 9), שאפשר להפוך לאוטומטית. |
| 3 | **Containerization (Docker)** | סביבת ריצה עקבית בין המחשב המקומי (Windows) ל-Render, מקטין "עובד אצלי אבל לא בפרודקשן". |
| 4 | **מנגנון rate-limiting/cooldown משותף (Redis או טבלת Supabase)** | ה-rate limiting הקיים (סעיף 8 פריט 3, סעיף 12.2) הוא state בזיכרון-תהליך בלבד - נמחק ב-restart ולא עובד נכון עם כמה instances במקביל. רלוונטי במיוחד אם מספר המשתמשים גדל מעבר ל-~100. |
| 5 | **הרצת `test_flow.ps1` בפועל** | הסקריפט נכתב אך מעולם לא הורץ סוף-לסוף מול Supabase אמיתי (ראו סעיף 2ב) - צעד קטן וזול לפני כל deployment. |

### 13.2 אבטחה (המשך לפריטים פתוחים מסעיף 7)

| # | רעיון | למה זה יכול לעזור |
|---|---|---|
| 6 | **Timeout מפורש + caching על שליפת מפתחות/קריאת האימות מול Supabase** | נשאר פתוח מסעיף 8, פריט 4 - תלוי בקובץ `db.py` שטרם נמסר. |
| 7 | **CORS מוגבל לדומיין אמיתי בלבד** | כרגע `allow_origins=["*"]` ב-`main.py` (סעיף 2ב) - חובה לצמצם לפני חשיפה לאינטרנט הרחב (תלוי ב-13.1 פריט 1). |
| 8 | **סקירת הרשאות/RLS מחדש לפני production** | ה-backend עוקף RLS לגמרי דרך `service_role key` (סעיף 3ב) - כדאי סבב בדיקה ידני שכל endpoint (כולל `recommendations.py` החדש, סעיף 12) עובר דרך סינון `user_id` מפורש. |
| 9 | **Secrets management** | כרגע `.env` מקומי בלבד - לפני production כדאי לוודא שמפתחות (Supabase service key, Gemini API key) מוזרקים דרך environment variables של הפלטפורמה (Render/Vercel), לא מחוץ לגרסת קוד בלבד. |
